...
module Facebooker
  ...
  class User
    ...
    FIELDS = [:status, :name, :pic_small] #ISS reduce fields
    attr_accessor :id, :session, :uid
    ...
    # Below is the method wrong
    ###
    ## ISS get friend online
    #def online_friends
    #  @online_friends = @session.post('facebook.users.getLoggedInUser', :session_key => @session.session_key).map do |uid|
    #    User.new(uid, @session)
    #  end
    #end
