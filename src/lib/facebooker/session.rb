...
module Facebooker
  ...
  class Session
    ...
    # ISS aggiunti
    def get_secret_from_session
      return @secret_from_session
    end
    def get_uid
      return @uid
    end
    ...
    def secure!
      ...
      # ISS want also to receive the secret of the session
      response = post 'facebook.auth.getSession', :auth_token => auth_token, :generate_session_secret => true
    ...
    def secure_with!(session_key, uid, expires, secret_from_session = nil)
      ...
      @uid = Integer(uid)
      # ISS: the new interface available is:
    ...
    private
      ...
      def signature_for(params)
        ...
        # ISS change here because I want to use always the secret key
        # to avoid that a session secret is used 
        #p "string to be signed: #{raw_string}"
        #p "signature_for (#{params[:method]}) with secret : #{@secret_key}"
